...
from app.schemas import TaskResponse, CreateTodo, UserResponse, AdminUserResponse
...
async def create_task(db: db_dependency, target_user_id, task: CreateTodo):
    return await admin_service.admin_create_task(db, target_user_id, task)

@router.post("/users/role/{user_id}", response_model=AdminUserResponse, status_code=200)
